    ...
    if perf_data:
        for p in params:
            ...
            if warning or critical:
                ...
                data+="warning_th=%s, critical_th=%s, "%(warning,critical)
    return data
...
def check_levels(param, warning, critical,message,ok=[]):
    if (numeric_type(critical) and numeric_type(warning)):
        if param >= critical:
            print "status=CRITICAL, " + message
            return 2
        elif param >= warning:
            print "status=WARNING, " + message
            return 1
        else:
            print "status=OK, " + message
            return 0
    else:
        ...
        print "status=CRITICAL, Unexpected value : %d" % param + "; " + message
        return 2
...
def main(argv):
    ...
    if action == "connections":
        ...
        check_page_faults_old(con, sample_time, warning, critical, perf_data)
...
def check_memory(con, warning, critical, perf_data,mapped_memory):
    ...
    try:
        ...
        if not data['mem']['supported'] and not mapped_memory:
            print "status=OK - Platform not supported for memory info"
            return 0
        #
        # convert to gigs
        #
        ...
        try:
            ...
            #message += " %.2fMB resident,"%( mem_resident)
        except:
            mem_resident = 0
            #message +=" resident unsupported,"
        try:
            mem_virtual = float(data['mem']['virtual'])
            #message +=" %.2fMB virtual," % mem_virtual
        except:
            mem_virtual=0
            #message +=" virtual unsupported,"
        try:
            mem_mapped = float(data['mem']['mapped'])
            #message +=" %.2fMB mapped," % mem_mapped
        except:
            mem_mapped = 0 
            #message +=" mapped unsupported,"
        try:
            mem_mapped_journal = float(data['mem']['mappedWithJournal'])
            #message +=" %.2fMB mappedWithJournal" % mem_mapped_journal
        except:
...
def check_opcounters(con, host, warning, critical,perf_data):
    ...
    if err1==0 and err2==0:
        ...
        message= "opcountersTotal=%d, opcountersInsert=%d, opcountersQuery=%d, opcountersUpdate=%d, opcountersDelete=%d, opcountersGetmore=%d, opcountersCommand=%d, " % tuple(per_minute_delta)
        message+=performance_data(perf_data,([(per_minute_delta[0],"opcountersReplTotal",warning,critical),(per_minute_delta[1],"opcountersReplInsert"),
...
def check_page_faults_old(con, sample_time, warning, critical, perf_data):
    warning = warning or 10
